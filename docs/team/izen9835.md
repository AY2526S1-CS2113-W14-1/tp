...
* **New Feature: Limit Exception Framework**
  * **What it does:** Implements three custom exceptions to enforce hard limits on object creation: maximum 9999 athletes, 999 sessions per athlete, and 99 exercises per session.
  * **Justification:** These limits prevent data structure overflow and ensure stable application behavior by catching capacity issues early with clear error messages.
  * **Highlights:**
    * `AthleteLimitReachedException` - Prevents exceeding the 9999 athlete maximum
    * `SessionLimitReachedException` - Prevents exceeding the 999 sessions per athlete maximum
    * `ExerciseLimitReachedException` - Prevents exceeding the 99 exercises per session maximum
    * Exceptions propagate to central error handler in `FitnessONE.java` for consistent user feedback
  * **Related:** `AthleteLimitReachedException.java`, `SessionLimitReachedException.java`, `ExerciseLimitReachedException.java`
